...
void led_streaming(uint8_t *data) // Stream data from HID Packets to Keyboard.
{
    ...
    for (uint8_t i = 0; i < numberofleds; i++) {
        ...
        if (isIndicator) {
            ...
            rgblight_setrgb_at(((uint16_t)r * current_val) / 255, ((uint16_t)g * current_val) / 255, ((uint16_t)b * current_val) / 255, index + i);
#elif defined(RGB_MATRIX_ENABLE)
            uint8_t current_val = rgb_matrix_get_val();
            rgb_matrix_set_color(index + i, ((uint16_t)r * current_val) / 255, ((uint16_t)g * current_val) / 255, ((uint16_t)b * current_val) / 255);
#endif
        }
    }
}
