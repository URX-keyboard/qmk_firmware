...
#define KB_LED_CAPS_LOCK_BIT 0x02
...
bool kb_get_caps_lock_state(void) {
    ...
    return (Keyboard_Status.System_Led_Status & KB_LED_CAPS_LOCK_BIT) != 0;
}
...
bool kb_led_update(led_t led_state) {
    // In wireless modes, mirror the RF module's reported Caps Lock state.
    if (Keyboard_Info.Key_Mode != QMK_USB_MODE) {
        if (led_state.caps_lock) {
            Keyboard_Status.System_Led_Status |= KB_LED_CAPS_LOCK_BIT;
        } else {
            Keyboard_Status.System_Led_Status &= ~KB_LED_CAPS_LOCK_BIT;
        }
    }
